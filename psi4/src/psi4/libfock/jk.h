...
class PSI_API DirectDFJK : public JK {
   protected:
    ...
    // We need mP functions to be called in the right order.
    //     an input to std::sort
    ...
    void compute_AO_block_Qpq(size_t start_Q, size_t stop_Q, double* ao_block,
                              std::vector<std::shared_ptr<TwoBodyAOInt>> eri);

    //	void compute_AO_block_p_pQq(size_t start_p, size_t stop_p, double* ao_block,
    //std::vector<std::shared_ptr<TwoBodyAOInt>> eri);

    void compute_dense_AO_block_p_pQq(size_t shell, double* ao_block, std::vector<std::shared_ptr<TwoBodyAOInt>> eri);

    void compute_AO_block_p_pQq_mn_sparse_set_mP(size_t shell, double* ao_block,
                                                 std::vector<std::shared_ptr<TwoBodyAOInt>> eri);

    void compute_AO_block_p_pQq_mn_mP_sparse(size_t shell, double* ao_block,
                                             std::vector<std::shared_ptr<TwoBodyAOInt>> eri);
    ...
    // Line 11 algorithm  8
    void X_accumulates_JU(size_t x_stop, size_t u_stop, size_t slice_size, size_t x_off, size_t u_off, size_t c_cols,
                          double* x, double* j, double* u);
    void X_accumulates_JU_pQq(size_t x_stop, size_t u_stop, size_t slice_size, size_t x_off, size_t u_off,
                              size_t c_cols, double* x, double* j, double* u);
    ...
    void set_uhf(std::string uhf) {
        if (uhf == "UHF") uhf_ = true;
    }
