...
void DirectDFJK::pytemplate(){
    ...
    for (int procs = 1; procs < 100; procs++ ) {
        printf("procs is %d \n", procs);
...
void DirectDFJK::compute_JK() {
	BB_ = false;
	//printf("nocc is %d \n", C_left_ao_[0]->ncol());
//    double* sabot = nullptr;
//	double saboteur;
//	if (n_col_last_ == 7) {saboteur = *sabot;}
...
double* DirectDFJK::get_metric_power(double power){
	...
	if (!pow_on) {
        //printf("inside DirectDFJK::get_metric_power, !pow_on\n");
...
//joejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoe
...
//printf("memory_ is %zu\n", memory_);
//printf("current_costs is %zu\n", current_costs );
//printf("max doubles used is %zu \n", biggest_block_ + current_costs );
...
outfile->Printf("preparing sparsity for shell %zu\n", shell);

	mP_func_map_pQq_[shell].clear();
	mP_shel_map_pQq_[shell].clear();
...
outfile->Printf("mP_func_map_pQq_[%zu].size() is %zu", shell, mP_func_map_pQq_[shell].size());
	...
	for ( size_t ShellP = 0; ShellP < Q_shells_; ShellP++ ) {
		...
		for (int i = 0; i < ShellP_count; i++ ) {colc[rank][i] = false;}
		shell_track[rank] = false;
...
//        loop is parallelized
		...
		for ( size_t ShellN = 0; ShellN < schwarz_shell_mask_pQq_[shell].size(); ShellN++ ) {
			...
			for ( size_t intp = 0; intp < ShellP_count; intp++ ) {
					for ( size_t intn = 0; intn < ShellN_count; intn++ ) {
						temp_ao[ intm*naux_*schwarz_dense_funcs_[shell]
                                + (intp + ShellP_start)*schwarz_dense_funcs_[shell]
                                + intn  ]
                        =
                        buffer[rank][ intp * Shell_ind_0
                                    + intm * ShellN_count
...
//	printf("ping\n");
    std::sort(mP_func_map_pQq_[shell].begin(), mP_func_map_pQq_[shell].end());//, std::greater<size_t> );
    std::sort(mP_shel_map_pQq_[shell].begin(), mP_shel_map_pQq_[shell].end());//, std::greater<size_t> );
	//printf("ping2\n");
...
//    this sparsity. we're going to repack the AOs for a pruned
//    tensor contraction.
		...
		for ( size_t Q_it = 0; Q_it < mP_size; Q_it++ ) {
			for (size_t nu_it = 0; nu_it < mn_size; nu_it++ ) {
					...
					temp_ao [ mu_it * naux_ * mn_size
							+ mP_func_map_pQq_[shell][Q_it] * mn_size 
							+ nu_it];
			}
		}
	}
...
eri[rank]->compute_shell( mP_shel_map_pQq_[shell][ShellP], 0, shell, schwarz_shell_mask_pQq_[shell][ShellN]);
			...
			for ( size_t intp = 0; intp < ShellP_count; intp++ ) {
					for ( size_t intn = 0; intn < ShellN_count; intn++ ) {
						ao_block[ intm*naux_*schwarz_dense_funcs_[shell] 
                                + (intp + ShellP_start)*schwarz_dense_funcs_[shell]
                                + intn  ]
                        =
                        buffer[rank][ intp * Shell_ind_0
...
//printf("nbf_*nbf_*naux_ is %zu \n", nbf_*nbf_*naux_);
//printf("nbf_*nbf_*naux_ is %zu \n", nbf_*nbf_*naux_);
//printf("x_size_ is %zu \n"        , x_size_);
//printf("x_size_ is %zu \n"        , x_size_);
//printf("naux_ is %zu \n"          , naux_);
//printf("naux_ is %zu \n"          , naux_);
...
//printf("storing %zu doubles\n", biggest_shell_ * naux_ * nbf_
//+ biggest_shell_ * naux_*nocc
//+ biggest_block_/nbf_*nocc
//+ biggest_block_/nbf_*nocc
//+ naux_
//);
...
void DirectDFJK::pQp_mn_sparse_set_mP() {
	outfile->Printf("entering pQp_mn_sparse_set_mP\n\n");
...
#pragma omp parallel num_threads(procs)
    ...
    //J_ao_[0]->save("/theoryfs2/ds/obrien/Debug/Psi4/directdfjk/direct_dfjk_j.txt", false, false, true);
    //K_ao_[0]->save("/theoryfs2/ds/obrien/Debug/Psi4/directdfjk/direct_dfjk_k.txt", false, false, true);
...
//printf("inside X_Block\n");

//joejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoejoe
    double* cmpq_invp = &CMPQ_inv_.front();
//joejoejoejoejoejoejoejoejoejoejoejoejeojeojeojoejoejoejoejoe
    ...
    if (!with_contraction){
        for (size_t shell_iter = Shell_starts_[block]; shell_iter <= Shell_stops_[block]; shell_iter++) {
            ...
            if (compute_k) {
				...
				for ( size_t func_it = 0; func_it < primary_->shell(shell_iter).nfunction(); func_it++ ){
					C_DGEMM('N', 'N', naux_, nocc, mP_func_map_pQq_[shell_iter].size(), 1.0, pruned_cm, mP_func_map_pQq_[shell_iter].size(), u + func_it * nocc * mP_func_map_pQq_[shell_iter].size(), nocc, 0.0, x + (primary_->shell(shell_iter).function_index() - primary_->shell(Shell_starts_[block]).function_index() + func_it)*nocc*naux_, nocc );
				}
            }
			for (size_t func_it = 0; func_it < primary_->shell(shell_iter).nfunction(); func_it++ ){
				prune_d( shell_iter, pruned_d, d + nbf_ * (func_it + primary_->shell(shell_iter).function_index()) );
        ...
        for (size_t shell_iter = Shell_starts_[block]; shell_iter <= Shell_stops_[block]; shell_iter++) {
			...
			if (compute_k) {
            	prune_c( shell_iter, nocc, pruned_c, c );
				prune_cmpq( shell_iter, met_m_0_5, pruned_cm);
				C_DGEMM('N', 'N', primary_->shell(shell_iter).nfunction() * mP_func_map_pQq_[shell_iter].size(), 
nocc, schwarz_dense_funcs_[shell_iter], 1.0, ao_block, schwarz_dense_funcs_[shell_iter], pruned_c, nocc, 0.0, u, nocc );
				for ( size_t func_it = 0; func_it < primary_->shell(shell_iter).nfunction(); func_it++ ){
			...
			for (size_t func_it = 0; func_it < primary_->shell(shell_iter).nfunction(); func_it++ ){
				prune_d( shell_iter, pruned_d, d + nbf_ * (func_it + primary_->shell(shell_iter).function_index()) );
				C_DGEMV( 'N', mP_func_map_pQq_[shell_iter].size(), schwarz_dense_funcs_[shell_iter], 1.0, ao_block + func_it * mP_func_map_pQq_[shell_iter].size() * schwarz_dense_funcs_[shell_iter], schwarz_dense_funcs_[shell_iter], pruned_d, 1, 0.0, pruned_coulomb_vector, 1); 
				unprune_V( shell_iter, coulomb_vector , pruned_coulomb_vector);
			}
        }
		outfile->Printf("about to try system solver");
        C_DGETRS( 'N', naux_, 1, metp, naux_, pert, coulomb_vector, naux_);
        for (size_t shell_iter = Shell_starts_[block]; shell_iter <= Shell_stops_[block]; shell_iter++) {
            compute_AO_block_p_pQq_mn_mP_sparse( shell_iter, ao_block, eri );
			prune_phi( shell_iter, coulomb_vector, pruned_coulomb_vector  );
			for (size_t func_it = 0; func_it < primary_->shell(shell_iter).nfunction(); func_it++){
				C_DGEMV( 'T', (int) mP_func_map_pQq_[shell_iter].size(), (int) schwarz_dense_funcs_[shell_iter], 1.0, ao_block + func_it * mP_func_map_pQq_[shell_iter].size() * schwarz_dense_funcs_[shell_iter], schwarz_dense_funcs_[shell_iter], pruned_coulomb_vector, 1, 0.0, pruned_j, 1 );
				unprune_J( shell_iter, j + nbf_ * (primary_->shell(shell_iter).function_index() + func_it), pruned_j);
			}
        }
    }
    ...
    switch (coul_work) {
        ...
        case 'P':
			...
			for (size_t shell_iter = Shell_starts_[block]; shell_iter <= Shell_stops_[block]; shell_iter++) {
				prune_phi( shell_iter, coulomb_vector, pruned_coulomb_vector  );
				for (size_t func_it = 0; func_it < primary_->shell(shell_iter).nfunction(); func_it++){
					C_DGEMV( 'T', (int) mP_func_map_pQq_[shell_iter][func_it], (int) schwarz_dense_funcs_[shell_iter], 1.0, ao_block + func_it * mP_func_map_pQq_[shell_iter][func_it] * schwarz_dense_funcs_[shell_iter], schwarz_dense_funcs_[shell_iter], coulomb_vector, 1, 0.0, pruned_j, 1 );
					unprune_J( shell_iter, j + nbf_ * (primary_->shell(shell_iter).function_index() + func_it), pruned_j);
				}
			}
            break;
